...
GLvoid UpdateGLScene(GLvoid) // Update code
{
	...
	//{
		...
		if (discCounter==12) // Yes they have stopped moving
		{
			...
			if (!player[activePlayer].GetDiscFiredStatus(currentDisc)) // Not been fired
			{
				if (!player[activePlayer].GetMovedToStartStatus(currentDisc)) // hasn't been moved to the
					// starting line
				{
					player[activePlayer].MoveDiscToThrowingLine(currentDisc);
					player[activePlayer].SetMovedToStartStatus(currentDisc, true);
				}
			}
			else
			{
	...
	if (myCamera.GetRotationStatus())
	{
			...
			// line
		{
			myTable.DrawThrowingLine(player[activePlayer].GetForce(),
				player[activePlayer].GetDiscPosition(currentDisc));
		}
...
int main (int argc, char **argv)
{
	...
	if (!CreateGLWindow("08214 - ACW - Jamie Sandell",640,480,16,fullscreen))
	{
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			if (keys[83]) // s - Variable coefficient of friction between the table table sides and each
				// disk
			{
				myTable.DecreaseSideFriction();
				keys[83]=FALSE;
			}
			if (keys[84]) // t - Toggle obstructions on/off
			{
				if (obstructionsEnabled)
				{
					obstructionsEnabled = false;
				}
				else
				{
			...
			if (keys[77]) // m - Move disc right
			{
				player[activePlayer].MoveDiscRight(currentDisc);
				keys[77]=FALSE;
			}
			if (keys[78]) // n - Move disc left
			{
				player[activePlayer].MoveDiscLeft(currentDisc);
				keys[78]=FALSE;
			}
			if (keys[VK_RIGHT])
			{
