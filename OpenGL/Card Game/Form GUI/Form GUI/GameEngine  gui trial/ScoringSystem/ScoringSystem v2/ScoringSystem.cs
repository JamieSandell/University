...
namespace CardGame
{
    public interface IScoringSystem
    {
        ...
        ///</summary>
        ///<returns>Nothing</returns>
        void drawHighScoreTable(CardGame.Text text);
    ...
    public class ScoringSystem:IScoringSystem
    {
        ...
        public void drawHighScoreTable(CardGame.Text text)
        {
            int x = -6, y = 4;

            text.RenderString(-2, 6, "High Score Table");

            foreach (ScoreEntry i in _mHighScoreTable)
            {
                text.RenderString(-3, y, i._mPlayersName);
                text.RenderString(0, y, i._mPlayersScore.ToString());
