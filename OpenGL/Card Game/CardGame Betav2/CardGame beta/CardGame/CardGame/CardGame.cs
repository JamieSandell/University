...
///////////Work Done: Worked on logic, scoring, and timer
///////////Signed Off: 12:05pm - Sunday 7th December 2008
//////////////////////////////////////////////////////////
...
namespace CardGame
{
    ...
    public class CardGame
    {
        ...
        private static bool gamePaused = false; //jason
        private static bool gameOver = false; //jason
        private static bool nameAdded = false; //Used to prevent name/score being 
        /////////////////////////////////////////added to high score table repeatedly on a single play //jason
        private static int initialPlayingTime = 90; //jason
        ...
        #region runGame()
        [STAThread]
        public static void runGame(Player inPlayer)
        {
            Glut.glutInit();
            ...
            myPlayer = inPlayer; // jason

            myScoringSystem.loadHighScoreTable(); //jason
            //timeRemainingTimer.StartTimer(); //jason
                        ...
                        if (!result)
                        {
                            ...
                            //{
                                ...
                                foreach (Card ray in cardRay)
                                {
                                    ...
                                    rayIndex++;
                                    if (rayIndex==cardRay.Length) // Jamie
                                    {
                                        rayIndex = cardRay.Length-1; // Jamie
                                    }
                                }
                                foreach (Card protection in cardProtection)
                                {
                                    ...
                                    protectionIndex++;
                                    if(protectionIndex==cardProtection.Length) // Jamie
                                    {
                                        protectionIndex = cardProtection.Length - 1; //
                                    }
                                }
                            }
                        }
                    ...
                    if (result) // found a match
                    {
                        ...
                        if (easyMode)
                        {
                            ...
                            if (pairsInARow >= 4)
                            {
                                ...
                                myPlayer.setScore(5 * 3); //jason
                            }
                            if (pairsInARow==0)
                            {
                                //score += 5;
                                myPlayer.setScore(5); //jason
                            }
                            //myUI.SetScore(score);
                            myUI.SetScore(myPlayer.getScore()); //jason
                        ...
                        if (mediumMode)
                        {
                            if ((pairsInARow > 0) && (pairsInARow < 4)) // if 4 multiplier = x3
                            {
                                ...
                                myPlayer.setScore(10 * 2); //jason
                            }
                            if (pairsInARow >= 4)
                            {
                                //score += 10 * 3; // 3x multiplier
                                myPlayer.setScore(10 * 3); // jason
                            }
                            if (pairsInARow == 0)
                            {
                                //score += 10; // double scoring
                                myPlayer.setScore(10); //jason
                            }
                            //myUI.SetScore(score);
                            myUI.SetScore(myPlayer.getScore()); //jason
                        }
                        if (hardMode)
                        {
                            if ((pairsInARow > 0) && (pairsInARow < 4)) // if 4 multiplier = x3
                            {
                                ...
                                myPlayer.setScore(15 * 2); //jason
                            }
                            if (pairsInARow >= 4)
                            {
                                //score += 15 * 3; // 3x multiplier
                                myPlayer.setScore(15 * 3); //jason
                            }
                            if (pairsInARow == 0)
                            {
                                //score += 15; // triple scoring
                                myPlayer.setScore(15); //jason
                            }
                            myUI.SetScore(myPlayer.getScore()); //jason
                        }
                    }
        ...
        private static void GameOver()
        {
            ...
            Gl.glVertex3f((float)_winL, (float)_winB, 0.0f); // Bottom Left

            Gl.glTexCoord2f(1.0f, 0.0f);
            Gl.glVertex3f((float)_winR, (float)_winB, 0.0f);// Bottom right

            Gl.glTexCoord2f(1.0f, 1.0f);
            Gl.glVertex3f((float)_winR, (float)_winT, 0.0f);// Top right

            Gl.glTexCoord2f(0.0f, 1.0f);
            Gl.glVertex3f((float)_winL, (float)_winT, 0.0f);// Top left
            Gl.glEnd();
            ...
            if (!nameAdded)
            {
                myScoringSystem.addToHighScoreTable(myPlayer.getName(), myPlayer.getScore());
                nameAdded = true;
            }

            if (!firstRun)
            {
                myScoringSystem.sortHighScoreTable();

                myScoringSystem.removeLowestScore();
                firstRun = true;
            }


            myScoringSystem.drawHighScoreTable(myText, _winL, _winR);

            myScoringSystem.saveHighScoreTable();
        ...
        {
            //jason
            myUI.SetTimeLeft((initialPlayingTime - timeRemainingTimer.GetTimeElapsed() / 1000)); // Update time remaining clock

            //jason
            if ((initialPlayingTime - timeRemainingTimer.GetTimeElapsed() / 1000) <= 0)
            {
                gameOver = true;
            }
            ...
            if (mediumMode) // initialise the mediumMode parts Jamie
            {
                if (firstRun)
                {
                    cardRay = new Card[18];
                    cardProtection = new Card[18];
                    for (int i = 0; i < cardRay.Length; i++)
                    {
                        cardRay[i] = new Card();
                        cardProtection[i] = new Card();
                    ...
                    for (int i = 0; i < cardRay.Length; i++)
                    {
                        ...
                        while (alreadyChosenThatCard) // need to pick another
                        {
                            number = rand.Next(0, 18);
                            chosenMedium[chosenCounter] = number;
                            for (int j = 0; j <= chosenCounter; j++)
                            {
                                if ((chosenMedium[j] == number) && (j != chosenCounter)) // already been chosen - don't
                                // check current card
                                {
                                    alreadyChosenThatCard = true;
                                    j = 0; // need to check the next new number against all others again
                    ...
                    chosenCounter = 0;
                    for (int i = 0; i < chosenMedium.Length; i++)
                    {
                        chosenMedium[i] = 7; // reset all of the chosen values to their original ones
                    }
                    // Set each card into a random position
                    Card[] cardRaySorting = new Card[cardRay.Length];
                    for (int i = 0; i < cardRay.Length; i++)
                    ...
                    alreadyChosenThatCard = true;
                    int y = 0; // used for positioning cards correctly
                    int y2 = 0; // Jamie used for position cards correctly
                    for (int i = 0; i < cardRay.Length; i++)
                    {
                        alreadyChosenThatCard = true;
                        while (alreadyChosenThatCard) // need to pick another
                        {
                            ...
                            for (int j = 0; j <= chosenCounter; j++)
                            {
                                ...
                                if (j == chosenCounter) // If here then a number that hasn't been chosen has been found
                                {
                                    ...
                                    float topY = 1.0f;
                                    float bottomY = 4.0f;
                                    if (chosenCounter>5) //next row down Jamie
                                    {
                                        topY = 6.0f;
                                        bottomY = 9.0f;
                                        x = y;
                                    ...
                                    if (chosenCounter>11) // next row down Jamie
                                    {
                                        topY = 11.0f;
                                        bottomY = 14.0f;
                                        x = y2;
                    ...
                    alreadyChosenThatCard = true;
                    y = 0; // used for positioning cards correctly
                    y2 = 0; // Jamie used for positioning cards correctly
                    for (int i = 0; i < cardProtection.Length; i++)
                    {
                        alreadyChosenThatCard = true;
                        while (alreadyChosenThatCard) // need to pick another
            ...
            if (hardMode)// initialise the hardMode parts Jamie
            {
                if (firstRun)
                {
                    cardRay = new Card[24];
                    cardProtection = new Card[24];
                    for (int i = 0; i < cardRay.Length; i++)
                    {
                        cardRay[i] = new Card();
                        cardProtection[i] = new Card();
                    ...
                    for (int i = 0; i < cardRay.Length; i++)
                    {
                        ...
                        while (alreadyChosenThatCard) // need to pick another
                        {
                            number = rand.Next(0, 24);
                            chosenHard[chosenCounter] = number;
                            for (int j = 0; j <= chosenCounter; j++)
                            {
                                if ((chosenHard[j] == number) && (j != chosenCounter)) // already been chosen - don't
                                // check current card
                                {
                                    alreadyChosenThatCard = true;
                                    j = 0; // need to check the next new number against all others again
                                ...
                                if (j == chosenCounter) // If here then a number that hasn't been chosen has been found
                                {
                                    ...
                                    // number, case 0
                                    {
                                        case 0:
                                            textureArray[0] = (int)_MCardBacksTexture[0];
                                            textureArray[1] = (int)_MCardHardTexture[0]; // Beta
                                            cardRay[i].SetProtection("Plastic Sheet");
                                            cardRay[i].SetRay("Beta");
                                            cardRay[i].SetTexture(textureArray);
                                            cardRay[i].SetType("Ray");
                                        ...
                                        case 1:
                                            textureArray[0] = (int)_MCardBacksTexture[0];
                                            textureArray[1] = (int)_MCardHardTexture[1]; // Gamma
                                            cardRay[i].SetProtection("Metal Sheet");
                                            cardRay[i].SetRay("Gamma");
                                            cardRay[i].SetTexture(textureArray);
                                            cardRay[i].SetType("Ray");
                                        ...
                                        case 2:
                                            textureArray[0] = (int)_MCardBacksTexture[0];
                                            textureArray[1] = (int)_MCardHardTexture[2]; // Infrared
                                            cardRay[i].SetProtection("Tinfoil");
                                            cardRay[i].SetRay("Infrared");
                                            cardRay[i].SetTexture(textureArray);
                                            cardRay[i].SetType("Ray");
                                        ...
                                        case 3:
                                            textureArray[0] = (int)_MCardBacksTexture[0];
                                            textureArray[1] = (int)_MCardHardTexture[3]; // Laser
                                            cardRay[i].SetProtection("Mirror");
                                            cardRay[i].SetRay("Laser");
                                            cardRay[i].SetTexture(textureArray);
                                            cardRay[i].SetType("Ray");
                                        ...
                                        case 4:
                                            textureArray[0] = (int)_MCardBacksTexture[0];
                                            textureArray[1] = (int)_MCardHardTexture[4]; // Ultraviolet
                                            cardRay[i].SetProtection("Sunglasses");
                                            cardRay[i].SetRay("Ultraviolet");
                                            cardRay[i].SetTexture(textureArray);
                                            cardRay[i].SetType("Ray");
                                        ...
                                        case 5:
                                            textureArray[0] = (int)_MCardBacksTexture[0];
                                            textureArray[1] = (int)_MCardHardTexture[5]; // X-Ray
                                            cardRay[i].SetProtection("Lead");
                                            cardRay[i].SetRay("X-Ray");
                                            cardRay[i].SetTexture(textureArray);
                                            cardRay[i].SetType("Ray");
                    ...
                    chosenCounter = 0;
                    for (int i = 0; i < chosenHard.Length; i++)
                    {
                        chosenHard[i] = 7; // reset all of the chosen values to their original ones
                    }
                    // Set each card into a random position
                    Card[] cardRaySorting = new Card[cardRay.Length];
                    for (int i = 0; i < cardRay.Length; i++)
                    ...
                    int y = 0; // used for positioning cards correctly
                    int y2 = 0; // Jamie used for position cards correctly
                    int y3 = 0; // Jamie used for position cards correctly
                    for (int i = 0; i < cardRay.Length; i++)
                    {
                        alreadyChosenThatCard = true;
                        while (alreadyChosenThatCard) // need to pick another
                        {
                            ...
                            for (int j = 0; j <= chosenCounter; j++)
                            {
                                ...
                                if (j == chosenCounter) // If here then a number that hasn't been chosen has been found
                                {
                                    ...
                                    if (chosenCounter > 17) // next row down Jamie
                                    {
                                        topY = 10.0f;
                                        bottomY = 12.0f;
                                        x = y3;
                    ...
                    y = 0; // used for positioning cards correctly
                    y2 = 0; // Jamie used for positioning cards correctly
                    y3 = 0; // Jamie used for positioning cards correctly
                    for (int i = 0; i < cardProtection.Length; i++)
                    {
                        alreadyChosenThatCard = true;
                        while (alreadyChosenThatCard) // need to pick another
            ...
            //jason
            if (noneFlippedProtectionCards.Count == 0 && noneFlippedRayCards.Count == 0)
            {

                gameOver = true;
                GameOver();

                myPlayer.setScore(initialPlayingTime - timeRemainingTimer.GetTimeElapsed() / 1000);
            }
        ...
        {
            //jason
            if (gamePaused) //Game paused so show appropriate message
            {
                Gl.glColor3f(1.0f, 0.0f, 0.0f);
                myText.RenderString(-5, 1, "Game Paused"); 
            }

            //jason
            else if (!gameOver) //Game isnt paused or over so continue to play the game
            {
                ...
                if (_MCurrentSceneToDraw == 1) // Draw the playing scene
                {
                    ...
                    if (mediumMode) // Draw the mediumMode version of the Scene Jamie
                    {
                        ///////////////// Ray Cards ///////////////////
                        ///////////////////////////////////////////////
                        // Draw the none flipped cards
                    ...
                    if (hardMode) // Draw the hardMode version of the scene Jamie
                    {
                        ///////////////// Ray Cards ///////////////////
                        ///////////////////////////////////////////////
                        // Draw the none flipped cards
                        ...
                        {
                            texture = myCard.GetTexture();
                            for (int i = 0; i < _MCardHardTexture.Length; i++)
                            {
                                // Find where the texture id is in the texture array
                                if (texture == _MCardHardTexture[i]) // found it
                                {
                                    texture = i;
                                    break; // No need to search anymore
                                }
                            }
                            Gl.glBindTexture(Gl.GL_TEXTURE_2D, _MCardHardTexture[texture]);
                            Gl.glBegin(Gl.GL_QUADS);
                            myCard.Draw();
                            Gl.glEnd();
                        }
            ...
            //jason
            else //The game is over so call GameOver()
            {
                GameOver();
            }
        ...
        private static void Keyboard(byte key, int x, int y)
        {
            switch (key)
            {
                ...
                case 112: //p Pause the game
                    if (!gamePaused) // If the game isn`t paused pause it
                    {
                        gamePaused = true;
                        timeRemainingTimer.StopTimer();
                    }
