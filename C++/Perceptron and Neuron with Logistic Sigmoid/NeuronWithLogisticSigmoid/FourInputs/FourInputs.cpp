...
double SimulatePerceptron(int inputSize, const double input[][4], const double weight[], int inputRow)
{
	double netSum = 0;
...
*/
inline void Training(double output, double weight[], const double input[][4], int inputSize, double learningRate,
			  const double target[], int numberOfInputs)
{
	double J = 5;
	double result;
	...
	while (J>quitCondition)
	{
		for (int i = 0; i < numberOfInputs - inputSize; i++) // for each sample,
			// numberOfInputs-2 because weights[i+j] will exceed the weight array bounds
		{
...
inline void LoadInputData(double input[][4], double target[], short numberOfInputs,
						  int numberOfTargets) // Load the input data
{
	...
	if (loadFile.is_open())
	{
		...
		for (int i = 0; i < numberOfInputs - 4; i++) // numberOfInputs - 3 because of the algorithm
														// below
		{
			input[i][0] = temp[i]; // i row, first column
			input[i][1] = temp[i+1]; // i row, 2nd column
			input[i][2] = temp[i+2]; // i row, 3rd column
			input[i][3] = temp[i+3]; // i row, 4th column
			target[i] = temp[i+4]; // target[0] = input[4][0]
		}
	}
	// TODO:
...
inline void WriteFile(double const target[])
{
	string dataToFile[3497]; // number of lines to write out, 3499 targets
	fstream writeFile;
	writeFile.open("targetsBeforeTraining.txt", ios::out);
...
int main()
{
	...
	const int inputSize = 4; // number of inputs per row
	const int numberOfTargets = numberOfInputs - inputSize; // numberOfTargets is numberOfInputs - 
										// number of inputs per row

	double input[numberOfInputs][4]; // [ROW][COL] - Don't train the 3501 line because 3501 does not have a target
	...
	double weight[3501]; // TODO: This needs looking at, because when the input is been carried over
	...
	output = SimulatePerceptron(inputSize, input, weight, 0);
	cout << "Output of sample should be: " << target[0] << " and is: " << output << "\n"
		<< "Accuracy is: " << abs(output - target[0]) << "\n" << "\n";
	...
	output = SimulatePerceptron(inputSize, input, weight, 1746);
	cout << "Output of sample should be: " << target[1746] << " and is: " << output << "\n"
		<< "Accuracy is: " << abs(output - target[1746]) << "\n" << "\n";

	output = SimulatePerceptron(inputSize, input, weight, 3496);
	cout << "Output of sample should be: " << target[3496] << " and is: " << output << "\n"
		<< "Accuracy is: " << abs(output - target[3496]);
