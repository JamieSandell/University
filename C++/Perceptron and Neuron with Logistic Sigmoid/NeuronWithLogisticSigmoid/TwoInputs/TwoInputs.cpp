...
double SimulatePerceptron(int inputSize, const double input[][2], const double weight[], int inputRow)
{
	double netSum = 0;
...
*/
inline void Training(double output, double weight[], const double input[][2], int inputSize, double learningRate,
...
int main()
{
	...
	const int inputSize = 2; // number of inputs per row
	...
	output = SimulatePerceptron(inputSize, input, weight, 1748);
	cout << "Output of sample should be: " << target[1748] << " and is: " << output << "\n"
		<< "Accuracy is: " << abs(output - target[1748]) << "\n" << "\n";

	output = SimulatePerceptron(inputSize, input, weight, 3498);
