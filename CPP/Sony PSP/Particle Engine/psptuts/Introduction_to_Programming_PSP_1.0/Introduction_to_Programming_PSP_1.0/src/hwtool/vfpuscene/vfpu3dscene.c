...
* All Rights Reserved.
*
* PSP Intro Hardware Tool Sample three - VFPU3dscene.c
*   This sample demonstrates 3D rendering of a lit scene consisting of a floor plane,
*	Two torii and a light representation, the light is rotated using VFPU library functions
*
*   Date            Design      Log
*  ----------------------------------------------------
...
// M A C R O S 
//-----------------------------------------------------------------------------
SCE_MODULE_INFO(VFPU3dscene, 0, 1, 1);
...
static ScePspFMatrix3	gRotMatrix;
...
int main(void)
{
	sceKernelChangeCurrentThreadAttr(0, SCE_KERNEL_TH_USE_VFPU); // Enable VFPU on this thread
...
static void Render(void)
{
	...
	sceVfpuMatrix3Unit(&gRotMatrix);
	sceVfpuMatrix3RotY(&gRotMatrix, &gRotMatrix, SCEGU_RAD(-gAngle));
	sceVfpuMatrix3Apply(&gLightPosVec, &gRotMatrix, &gLightPosVec);
	sceGuLight(0, SCEGU_LIGHT_POINT, SCEGU_DIFFUSE_AND_SPECULAR, &gLightPosVec);
...
static void ReadPad(void)
{
	...
	if (ret != 0)
	{
		...
		if (!dead)
			sceKernelPowerTick(SCE_KERNEL_POWER_TICK_LCDONLY);
