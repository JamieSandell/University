...
//void Particle::Draw(void) const
{
	...
	glColor4f(_colour[0], _colour[1], _colour[2], _life);
	...
	//glMaterialfv(GL_FRONT_AND_BACK, GL_DIFFUSE, circleDiffuse);
		...
		glTexCoord2f(1.0,1.0f); glVertex3f(_position[0]+5.0f,_position[1]+1.8f,_position[2]); // Top Right
		glTexCoord2f(0.0,1.0f); glVertex3f(_position[0]-1.8f,_position[1]+1.8f,_position[2]); // Top Left
		glTexCoord2f(1.0,0.0f); glVertex3f(_position[0]+5.0f,_position[1]-1.8f,_position[2]); // Bottom Right
		glTexCoord2f(0.0,0.0f); glVertex3f(_position[0]-1.8f,_position[1]-1.8f,_position[2]); // Bottom Left
...
void Particle::Update(void)
{
	...
	_position[0] = _position[0]+_speed.X()/(1.0*1000); // Change 1 to slowdown variable, Move On The X Axis By X Speed
	_position[1] = _position[1]+_speed.Y()/(1.0*1000); // Change 1 to slowdown variable, Move On The Y Axis By Y Speed
	_position[2] = _position[2]+_speed.Z()/(1.0*1000); // Change 1 to slowdown variable, Move On The Z Axis By Z Speed
	...
	*/
	...
	if (_life<0.0f)
	{
		...
		_position[0] = rand () % (high - low + 1) + low;
		_position[1] = 0.0;
		_position[2] = 0.0;
		...
		_colour[0] = rand()/(float(RAND_MAX)+1);
		_colour[1] = rand()/(float(RAND_MAX)+1);
		_colour[2] = rand()/(float(RAND_MAX)+1);
	}
