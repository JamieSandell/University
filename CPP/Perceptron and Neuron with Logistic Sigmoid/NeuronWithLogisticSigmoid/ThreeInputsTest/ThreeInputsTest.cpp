...
const static int inputSize = 3; // number of inputs per row

// Random number with Gaussian Distribution
inline static void RandomWeight(double minimumValue, double maximumValue, double weight[][3],
								int numberOfInputs)
{
	//srand(timeGetTime());
	for (int i = 0; i < numberOfInputs; i++)
	{
		for (int j = 0; j < inputSize; j++)
		{
			for (int k = 0; k < inputSize; k++)
			{
				...
				weight[i][j][k] = minimumValue + (maximumValue-minimumValue)*random;
			}
		}
	}
}
...
double SimulatePerceptron(int inputSize, const double input[][2], const double weight[][3], int inputRow)
{
	double netSum = 0;
