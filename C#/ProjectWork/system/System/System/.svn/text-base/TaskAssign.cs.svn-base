...
namespace SystemManagement
{
    ...
    {      
        ...
        string SetPercentageComplete(int inPercentageComplete);

        string CheckPercentageComplete(int inPercentageComplete);

        int GetPercentageComplete();

        string StartTimer(ITaskAssign taskAssign);

        string SetTimersStatus(bool status);

        string StopTimer(ITaskAssign taskAssign);
    ...
    public class TaskAssign : ITaskAssign
    {
        ...
        private DateTime dateTimerStarted;
        private DateTime dateTimerStopped;
        ...
        public string SetPercentageComplete(int inPercentageCompletion)
        {
        ...
        public string CheckPercentageComplete(int inPercentageCompletion)
        {
        ...
        public string StartTimer(ITaskAssign taskAssign)
        {
            if (!timerStarted)
            {
                dateTimerStarted = dateTimerStarted.Date;
                SetTimersStatus(true);
                return ""; // Everything went ok
            }
            return "This task's timer has already started.";
        }

        public string SetTimersStatus(bool status)
        {
            timerStarted = status;
            return ""; // Everything went OK
        }

        public string StopTimer(ITaskAssign taskAssign)
        {
            if (timerStarted)
            {
                dateTimerStopped = dateTimerStopped.Date;
                taskAssign.SetTimersStatus(false);
                TimeSpan span = dateTimerStopped.Subtract(dateTimerStarted);
                string result = taskAssign.AppendHours(span);
                return result;
            }
            else
            {
                return "This task's timer hasn't been started.";
            }
        }
    }    
}
